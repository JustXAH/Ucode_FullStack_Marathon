...
function animalCheck() {
    ...
    let reqAnimal = RegExp('^[a-zA-Z]+$');

    if (animal.length > 20 || !reqAnimal.test(animal)) {
        alert('ERROR: Accepts only one word, which consists ' +
            'only of Latin letters and its length does not exceed 20' +
            ' characters.');
        animal = null;
    }
    else {
...
function genderCheck() {
    let gender =  prompt('Is the superhero male or female? Leave blank if' +
        ' unknown or other.', '');
    let reqGender = RegExp('^male$|^female$|^$', 'i')

    if (!reqGender.test(gender)) {
        alert('ERROR: Accepts only male, female gender or blank (not case' +
            ' sensitive)!');
        gender = null;
    }
    else {
...
function ageCheck() {
    ...
    if (age.length > 5 || !reqAge.test(age)) {
        alert('ERROR: Accepts only digits, cannot start with a zero, no' +
